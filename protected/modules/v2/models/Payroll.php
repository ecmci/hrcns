...
class Payroll extends CActiveRecord
{
	...
	/**
	 * Sets the PTO Eligibility; make sure only full time are eligible
	 */ 
	private function setPTOEligibility($employment){
		switch($employment->status){
			case 'FULL_TIME':
				//do nothing, assumed pto_effective_date and is_pto_eligible fields populated upon validation
			break;
			default: 
			$this->is_pto_eligible = '0';
			$this->pto_effective_date = null;
		}		
	}
	...
	public function rules()
	{
		...
		return array(
			//all scenario		
			array('rate_type, rate_proposed, w4_status', 'required'),
	...
	public function validatePTO($employment){
		$v0 = true;
		$v1 = true;
		if($employment->status == 'FULL_TIME' and $this->is_pto_eligible != '1'){
			$this->addError('is_pto_eligible','A full time employee must be eligible for PTO.');
			$v0 = false;
		}
		if($this->is_pto_eligible == '1' and empty($this->pto_effective_date)){
			$this->addError('pto_effective_date','PTO Effective Date is required.');
			$v1 = false;
		}
		//if all good, set the pto elibility
		$this->setPTOEligibility($employment);
		return $v0 and $v1;
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'emp_id' => 'Emp',
			'is_pto_eligible' => 'Eligible for PTO',
