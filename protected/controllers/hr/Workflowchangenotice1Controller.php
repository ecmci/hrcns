...
class Workflowchangenotice1Controller extends Workflowchangenotice0Controller{
  ...
  public function actionView($id){
    $this->redirect(array('/v2/notice/review','id'=>$id));   
  }
  ...
  public function actionView0($id){
    ...
    if(Yii::app()->user->getState('hr_group') == 'CORP' and $notice->status == 'WAITING'){		
		$this->redirect(array('/hr/workflowchangenotice/finalize','id'=>$notice->id));
    }elseif($notice->returnToBom()){
      //$this->redirect(array('/hr/employee/modify','id'=>$notice->id));
      $this->redirect(array('/hr/workflowchangenotice/redo','id'=>$notice->id));
    }else{
