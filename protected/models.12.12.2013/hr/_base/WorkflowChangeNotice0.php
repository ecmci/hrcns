...
//       $diff = Helper::dateDiff(date('Y-m-d',time()),$this->timestamp); 
//       Yii::log('DIFF: '.print_r($diff,true),'error','app');
//       return $diff['days'] > self::$FLAG_DAYS or $diff['months'] > self::$FLAG_MONTHS or $diff['years'] > self::$FLAG_YEARS;
...
//     }
  ...
  public function afterFind(){
    ...
    //$this->employee = Employee::model()->find("emp_id = '".$emp_id."'")->getFullName();
    //$this->position = ($this->employmentProfile->position_code) ? Position::model()->find("code = '".$this->employmentProfile->position_code."'")->name : '(Pending...)';
    $this->comments = $this->getLastGroupComment();
    $this->corp_id = User::getName($this->corp_id);
    //$diff = Helper::dateDiff(date('Y-m-d',time()),$this->timestamp);
    $days_ago = time() - strtotime($this->timestamp);
    $days_ago = floor($days_ago / (60 * 60 * 24));
    $this->created_days_ago = $days_ago == 0 ? 'Today' : $days_ago.' days ago' ;
